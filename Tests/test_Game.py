...
class TestGame(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.game = Game(
            self.rows, self.cols, self.k, self.player1, self.player2
    ...
    def test_board_initialization(self):
        ...
        """
        self.assertEqual(
            self.game.board.grid.shape, (self.rows, self.cols)
        )  # Check board dimensions
        self.assertTrue(
            (self.game.board.grid == 0).all()
        )  # Check if board starts empty
    ...
    def test_bot_move(self):
        ...
        row, col = bot.make_move(
            bot.number, self.game.board, bot.mode
        )  # Get bot's move
        ...
        self.assertEqual(
            self.game.board.grid[row, col], 0
        )  # Ensure the chosen cell was originally empty
    ...
    def test_player_win(self):
        ...
        self.assertTrue(
            self.game.board.has_won(1)
        )  # Check if Player 1 is recognized as the winner
